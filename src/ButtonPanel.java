...
public ArrayList<String> getWords(Tile[][] theBoard) {
...
boolean isAdjacent (int x, int y, Tile[][] theBoard) {
        if (x == 7 && y == 7) return true;
                ...
                if (x == 0)         {xmin = x; xmax = x + 1; }
                else if (x == 14)   {xmin = x - 1; xmax = x; }
                if (y == 0)         {ymin = y; ymax = y + 1; }
                else if (y == 14)   {ymin = y - 1; ymax = y; }
...
public boolean checkSingleLine() {   // check if placed in single line
...
public boolean checkAdjacent() {
        ...
        for (String s: c) {
                ...
                scanner.next();
                Boolean bool = new Boolean(scanner.next());
        ...
        for (String s: b) {
                ...
                if (word.length() > 1) {
                        for (int i = y - word.length() +1; i < y + 1; i++) {
                                wordMulti = wordMulti * theBoard[x][i].tileType.getWordMultiplier();
                                score = score + theBoard[x][i].tileValue *
                                        theBoard[x][i].tileType.getLetterMultiplier();
                        }
